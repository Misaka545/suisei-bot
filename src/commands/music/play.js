...
async function extractYouTubePlaylist(url, max = MAX_ENQUEUE) {
  ...
  const out = await ytdlp(url, {
    dumpSingleJson: true, flatPlaylist: true, playlistEnd: max, skipDownload: true, noWarnings: true, quiet: true, jsRuntimes: "node"
  });
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (!voiceChannel) return interaction.editReply("❌ You must join a voice channel first!");
    if (!inputRaw) return interaction.editReply("❌ Please provide a Spotify or YouTube link.");
    await interaction.editReply(`🎶 Loading: ${inputRaw}`);
    ...
    try {
      ...
      return interaction.editReply(followMsg);
    } catch (err) {
      console.error(err);
      return interaction.editReply(`⚠️ Error: Could not play/enqueue.\n\`\`\`${err.message || err}\`\`\``);
    }
  },
};
